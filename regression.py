...
class OnlineLinearRegressionWithAbsoluteLoss:
    def __init__(self, dim):
        ...
        self._losses = []  # cumulative loss
        self._lin_losses = []  # linearized cumulative loss
...
class OnlineGradientDescent(OnlineLinearRegressionWithAbsoluteLoss):
    ...
    def get_action(self, h_t=None):
        ...
        if t > 1:
            ...
            w = self.w - self.lr(t) * self.g_prev  # no projection since unconstrained
            return w
        else:
            return self.w
...
class CoinBetting(OnlineLinearRegressionWithAbsoluteLoss):
    ...
    def get_action(self, h_t=None):
        ...
        w = v * (self.init_wealth - self.lin_losses.sum())
...
class CoinBettingWithFixedDepthSideInformation(CoinBettingWithQuantizedSideInformation):
    ...
    def get_side_information(self):
        h_t = tuple(self.suffix)  # h_t = Q(g_{t-1}^{t-D})
        return h_t

    def update(self, g_t, h_t=None):
        super().update(g_t, h_t)
        self.suffix = self.suffix[1:] + [self.quantize(g_t)]  # update the suffix


class CoinBettingWithQuantizedHint(CoinBettingWithQuantizedSideInformation, Quantizer):
    def __init__(self, dim, init_wealth=1, quantizer_vector=None):
        """
        Implement coin betting with a fixed order Markov type side information with a binary quantizer
    ...
    def get_side_information(self, x_t, y_t):
        h_t = self.quantize(self.subgradient(self.w, x_t, y_t))
        return h_t
...
class ContextTreeWeighting(OnlineLinearRegressionWithAbsoluteLoss, Quantizer):
    ...
    def update(self, g_t, suffix=None):
        # Update
        self.context_tree.update(state=suffix, g_new=-g_t)
        self.suffix = list(suffix[1:]) + [self.quantize(g_t)]  # update the suffix
        self.suffix = self.suffix[:self.max_depth]  # to handle the degenerate case when max_depth=0
